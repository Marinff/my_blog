...
updated: '2025-05-08 23:54:02'
permalink: /post/hashcat-z1yd89e.html
comments: true
toc: true
---
...
很重要的一个工具，但是一直没有认真学，还是得研究一下

|参数|意义|
| ---------------| --------------------------------------------------------------------------|
|-a|指定要使用的爆破模式，值参考后面的攻击模式|
|-m|指定要破解的hash类型，默认是MD5|
|-o|指定破解后的hash和密码存放的位置，可以用它把破解成功的hash写到指定的文件中|
|--show|只显示破解成功后的hash及对应的明文信息|
|--userrname|忽略hash表中的用户名|
|--force|忽略警告|
|-i|启动增量破解模式，让hashcat在指定的密码长度范围内执行破解过程|
|--increment-min|设置密码最小长度，配合增量破解模式|
|--increment-max|设置密码最大程度，配合增量破解模式|
|-O|限制密码长度|
|-d|这个好像是选择爆破用的是cpu或gpu|
...
![](https://pic.imgdb.cn/item/66ee8710f21886ccc02763c2.png)

```
hashcat --hash-info | grep '$2a' -n10
```
...
![](https://pic.imgdb.cn/item/66ee82f8f21886ccc0234fb9.png)
